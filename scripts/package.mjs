...
const mode = process.argv.slice(2).filter(arg => arg !== '--native')[0] || 'production'
...
run(process.execPath, [resolve(root, native ? 'scripts/build-native.mjs' : 'scripts/build.mjs'), mode])
await mkdir(resolve(root, 'artifacts'), { recursive: true })
const name = `dashboard-${native ? 'native-' : ''}${mode}.tar.gz`
